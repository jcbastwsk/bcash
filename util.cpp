...
void posix_locking_callback(int mode, int type, const char* file, int line)
{
    if (mode & CRYPTO_LOCK)
...
public:
    CInit()
    {
        ...
        CRYPTO_set_locking_callback(posix_locking_callback);

        // Seed random number generator with system entropy sources
        RAND_poll();
#endif
...
#else
    // Seed with high-resolution monotonic clock
    struct timespec ts;
...
#else
        // Seed with data from /dev/urandom
        unsigned char pdata[2048];
        memset(pdata, 0, sizeof(pdata));
        FILE* furandom = fopen("/dev/urandom", "rb");
        if (furandom)
        {
            unsigned long nSize = fread(pdata, 1, sizeof(pdata), furandom);
            fclose(furandom);
            if (nSize > 0)
            {
                uint256 hash;
                ...
                hash = 0;
                memset(pdata, 0, nSize);
                printf("RandAddSeed() got %lu bytes from /dev/urandom\n", nSize);
            }
        }
#endif
    }
...
{
    n /= CENT;
    string str = strprintf("%" PRId64 ".%02" PRId64, (n > 0 ? n : -n)/100, (n > 0 ? n : -n)%100);
...
uint64 GetRand(uint64 nMax)
{
    ...
    uint64 nRange = (ULLONG_MAX / nMax) * nMax;
    uint64 nRand = 0;
    do
...
void AddTimeData(unsigned int ip, int64 nTime)
{
    ...
    printf("Added time data, samples %d, ip %08x, offset %+" PRId64 " (%+" PRId64 " minutes)\n", vTimeOffsets.size(), ip, vTimeOffsets.back(), vTimeOffsets.back()/60);
