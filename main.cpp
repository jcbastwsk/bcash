...
string DateTimeStr(unsigned int nTime)
{
    time_t t = nTime;
...
{
    // Cap orphan transactions to prevent memory exhaustion
    if (mapOrphanTransactions.size() >= 100)
    {
        // Evict a random orphan
        map<uint256, CDataStream*>::iterator it = mapOrphanTransactions.begin();
        advance(it, GetRand(mapOrphanTransactions.size()));
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        txNew.vout[0].scriptPubKey = CScript() << CBigNum("0x04678AFDB0FE5548271967F1A67130B7105CD6A828E03909A67962E0EA1F61DEB649F6BC3F4CEF38C4F35504E51EC112DE5C384DF7BA0B8D578A4C702B6BF11D5F") << OP_CHECKSIG;
        CBlock block;
...
bool AlreadyHave(CTxDB& txdb, const CInv& inv)
{
    ...
    case MSG_TX:
        CRITICAL_BLOCK(cs_mapTransactions)
        {
            if (mapTransactions.count(inv.hash))
                return true;
        }
...
bool ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    for (int i = 0; i < min(vRecv.size(), (size_t)25); i++)
    ...
    else if (strCommand == "news")
    {
        ...
        CNewsItem item;
        vRecv >> item;
        CInv inv(MSG_NEWS, item.GetHash());
        pfrom->AddInventoryKnown(inv);
    ...
    else if (strCommand == "newsvote")
    {
        CDataStream vMsg(vRecv);
        CNewsVote vote;
        vRecv >> vote;
        CInv inv(MSG_NEWSVOTE, vote.GetHash());
        pfrom->AddInventoryKnown(inv);
...
bool BitcoinMiner()
{
    ...
    while (fGenerateBitcoins)
    {
        ...
        // Bgold merge-mining: create a bgold block and embed commitment in coinbase
        ...
        // hashBcashBlock will be filled in after we know the block hash

        // Add OP_RETURN bgold commitment as second output
        // We use a placeholder hash for now; it gets updated below
        CTxOut txoutBgold;
        txoutBgold.nValue = 0;
        ...
        printf("\n\nRunning BcashMiner with %d transactions in block\n", pblock->vtx.size());
        ...
        loop
        {
...
#else
                // nice(0) is not useful here; already at low priority
#endif
                CRITICAL_BLOCK(cs_main)
                {
                    // Save key
                    ...
                    if (!ProcessBlock(NULL, pblock.release()))
                        printf("ERROR in BcashMiner, ProcessBlock, block not accepted\n");
                }
#ifdef _WIN32
                SetThreadPriority(GetCurrentThread(), THREAD_PRIORITY_LOWEST);
#else
            ...
            if ((++tmp.block.nNonce & 0x3ffff) == 0)
            {
                ...
                if (nElapsed > 0)
                {
                    double dHashesPerSec = (double)tmp.block.nNonce / nElapsed;
                    printf("\r  Mining height %d | %s | %.0f hash/s | nonce 0x%08x   ",
                        nBestHeight + 1,
