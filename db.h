...
public:
    bool WriteProduct(uint256 hash, const CProduct& product)
    {
        return Write(make_pair(string("product"), hash), product);
    }

    bool ReadProduct(uint256 hash, CProduct& product)
    {
        return Read(make_pair(string("product"), hash), product);
    }

    bool WriteOrder(uint256 hash, const CWalletTx& order)
    {
        return Write(make_pair(string("order"), hash), order);
    }

    bool ReadOrder(uint256 hash, CWalletTx& order)
    {
        return Read(make_pair(string("order"), hash), order);
    }
};
